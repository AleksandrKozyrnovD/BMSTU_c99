...
START_TEST(test_pos_1_key)
{
    ...
    int rc = key(array, array + 5, &filtered_beg, &filtered_end);
    ck_assert_int_eq(rc, EXIT_SUCCESSFUL);

    ck_assert(filtered_beg != NULL);
    ck_assert_uint_eq(3, filtered_end - filtered_beg);
    ck_assert_int_eq(*(filtered_beg + 0), 2);
    ck_assert_int_eq(*(filtered_beg + 1), 3);
    ck_assert_int_eq(*(filtered_beg + 2), 4);
    free(filtered_beg);
}
END_TEST
...
START_TEST(test_pos_2_key)
{
    ...
    ck_assert(filtered_beg != NULL);
    ck_assert_uint_eq(3, filtered_end - filtered_beg);
    ck_assert_int_eq(*(filtered_beg + 0), 4);
    ck_assert_int_eq(*(filtered_beg + 1), 3);
    ck_assert_int_eq(*(filtered_beg + 2), 2);
    free(filtered_beg);
}
END_TEST
...
START_TEST(test_pos_3_key)
{
    ...
    int rc = key(array, array + 3, &filtered_beg, &filtered_end);
    ck_assert_int_eq(rc, EXIT_SUCCESSFUL);
...
START_TEST(test_neg_1_key)
{
    ...
    int rc = key(array, array, &filtered_beg, &filtered_end);
    ck_assert_int_eq(rc, ERR_IO);
    ck_assert(filtered_beg == NULL);

    int arr[] = { 1, 2, 3 };
    rc = key(arr, arr + 3, NULL, NULL);
    ck_assert_int_eq(rc, ERR_IO);
}
END_TEST
...
{
    //source pointers incorrectly used, as example begin is later than end.
    int *filtered_beg = NULL, *filtered_end;
    int array[] = { 1, 2, 3 };

    int rc = key(array + 3, array, &filtered_beg, &filtered_end);
    ck_assert_int_eq(rc, ERR_IO);
    ck_assert(filtered_beg == NULL);
}
END_TEST
...
START_TEST(test_neg_3_key)
{
    ...
    int rc = key(array, array + 2, &filtered_beg, &filtered_end);
...
START_TEST(test_neg_4_key)
{
    ...
    int rc = key(array, array + 4, &filtered_beg, &filtered_end);
    ck_assert_int_eq(rc, EMPTY);
    ck_assert(filtered_beg == NULL);
}
END_TEST
